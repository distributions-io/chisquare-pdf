...
* @param {Number} [options.df=1] - degrees of freedom
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'df' ) ) {
		opts.df = options.df;
		if ( !isPositive( opts.df ) ) {
			return new TypeError( 'pdf()::invalid option. `df` parameter must be a positive number. Option: `' + opts.df + '`.' );
		}
	}
