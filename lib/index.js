...
* @param {Number} [opts.df=1] - degrees of freedom
...
function pdf( x, options ) {
	...
	opts.df = typeof opts.df !== 'undefined' ? opts.df : 1;

	if ( isNumber( x ) ) {
		return pdf1( x, opts.df );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.df );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.df, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.df, opts.accessor );
		}
		return pdf2( out, x, opts.df );
	}
	return NaN;
} // end FUNCTION pdf()
